...
extern const uint8_t wheelchair_detect_espdl[] asm("_binary_wheelchair_detect_espdl_start");
static const char *path = (const char *)wheelchair_detect_espdl;
...
{
    switch (model_type) {
    case model_type_t::ESPDET_PICO_224_224_WHEELCHAIR:
...
void ESPDetDetect::load_model()
{
    switch (m_model_type) {
    case model_type_t::ESPDET_PICO_224_224_WHEELCHAIR:
#if CONFIG_FLASH_ESPDET_PICO_224_224_WHEELCHAIR || CONFIG_WHEELCHAIR_DETECT_MODEL_IN_SDCARD
        m_model = new espdet_detect::ESPDet("espdet_pico_224_224_wheelchair.espdl", m_score_thr[0], m_nms_thr[0]);
#else
        ESP_LOGE("wheelchair_detect", "espdet_pico_224_224_wheelchair is not selected in menuconfig.");
#endif
        break;
    }
}
